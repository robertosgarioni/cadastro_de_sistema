...
if (linkExclusao != null) { 
	for ( var i = 0; i < linkExclusao.length; i++ ) {
		(function(i){
			var idAluno = linkExclusao[i].getAttribute('rel');

			if (linkExclusao[i].addEventListener){
		    	linkExclusao[i].addEventListener("click", function(){confirmaExclusao(idAluno);});
			}else if (linkExclusao[i].attachEvent) { 
				linkExclusao[i].attachEvent("onclick", function(){confirmaExclusao(idAluno);});
			}
		})(i);
	}
}
...
function confirmaExclusao(idAluno){
	...
	if (retorno){
	    ...
	    formulario.action = "cadastrar_aluno.php";
